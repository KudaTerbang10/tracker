...
class ManifestDetailScreen extends ConsumerWidget {
  ...
  Widget _buildContent(
      BuildContext context, Manifest manifest, WidgetRef ref) {
                  ...
                  children: [
                    ...
                    IntrinsicHeight(
                      child: Row(
                        ...
                        children: [
                          Expanded(child: _detailInfoCard(context, 'Penerima', tx.penerimaName, tx.penerima['phone'] as String? ?? '', tx.penerimaAddress, Icons.call_received_rounded, AppTheme.secondary)),
                          const SizedBox(width: 8),
                          Expanded(child: _detailInfoCard(context, 'Pengirim', tx.pengirimName, tx.pengirim['phone'] as String? ?? '', tx.pengirim['address'] as String? ?? '', Icons.send_rounded, AppTheme.primary)),
                        ],
                      ),
                    ),
                    const SizedBox(height: 12),
                    Card(
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          children: [
                            Expanded(child: _detailInfoCell(Icons.scale_rounded, 'Berat', tx.beratLabel)),
                            Container(width: 1, height: 40, color: const Color(0xFFE2E8F0)),
                            Expanded(child: _detailInfoCell(Icons.inventory_2_rounded, 'Jumlah Koli', '${tx.jumlahKoli} koli')),
                            if (tx.biayaKirim > 0) ...[
                              Container(width: 1, height: 40, color: const Color(0xFFE2E8F0)),
                              Expanded(child: _detailInfoCell(Icons.payments_rounded, 'Biaya Kirim', fmt.format(tx.biayaKirim))),
                            ],
                          ],
                        ),
                      ),
                    ...
                    if (showDriver || (tx.namaPenerimaAkhir != null && tx.namaPenerimaAkhir!.isNotEmpty)) ...[
                      ...
                      IntrinsicHeight(
                        child: Row(
                          ...
                          children: [
                            if (showDriver)
                              Expanded(child: _detailInfoCard(context, 'Driver Kurir', tx.namaDriver!, tx.kontakDriver ?? '', '', Icons.directions_car_filled_rounded, Colors.amber.shade700)),
                            if (showDriver && tx.namaPenerimaAkhir != null && tx.namaPenerimaAkhir!.isNotEmpty)
                              const SizedBox(width: 8),
                            if (tx.namaPenerimaAkhir != null && tx.namaPenerimaAkhir!.isNotEmpty)
                              Expanded(child: _detailInfoCard(context, 'Diterima oleh', tx.namaPenerimaAkhir!, '', '', Icons.check_circle_rounded, Colors.green.shade600)),
                          ],
                        ),
                      ),
                    ],
  ...
  Widget _detailInfoCard(BuildContext ctx, String title, String name, String phone, String address, IconData icon, Color accent) {
    return Card(
      color: Colors.white,
          ...
          Container(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 6),
                Text(title, style: TextStyle(color: accent, fontWeight: FontWeight.w700, fontSize: 12)),
              ],
            ),
          ),
          Expanded(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  Text(name, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: Color(0xFF0F172A)), maxLines: 2, overflow: TextOverflow.ellipsis),
                  if (address.isNotEmpty) ...[
                    const SizedBox(height: 4),
                    Flexible(
                      child: Text(address, style: const TextStyle(fontSize: 11, color: Color(0xFF475569), height: 1.3), maxLines: 3, overflow: TextOverflow.ellipsis),
                    ),
                  ],
                ],
              ),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 4),
                  Expanded(child: Text(phone, style: const TextStyle(color: Color(0xFF475569), fontWeight: FontWeight.w600, fontSize: 11))),
                  GestureDetector(
                    onTap: () {
                      Clipboard.setData(ClipboardData(text: phone));
                      ScaffoldMessenger.of(ctx).showSnackBar(const SnackBar(content: Text('Nomor telepon disalin'), duration: Duration(seconds: 1)));
                    },
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(value, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 13, color: Color(0xFF0F172A)), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis),
        ],
      ),
    );
  }
  ...
  Future<void> _printManifest(BuildContext context, Manifest m) async {
  ...
  pw.Widget _pdfCell(String text,
      {bool header = false, pw.Font? font}) {
    return pw.Padding(
